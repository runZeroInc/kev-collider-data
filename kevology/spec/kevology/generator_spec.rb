...
RSpec.describe Kevology::Generator do
  ...
  describe '.parse_epss_snapshot' do
    it 'streams gzip snapshots and skips comment lines' do
      Dir.mktmpdir('epss_snapshot_') do |tmpdir|
        path = File.join(tmpdir, 'epss_scores.csv.gz')
        Zlib::GzipWriter.open(path) do |gz|
          gz.write("# generated by test\n")
          gz.write("cve,epss,percentile\n")
          gz.write("CVE-2021-44228,0.97,0.999\n")
        end

        result = described_class.parse_epss_snapshot(path)
        expect(result['CVE-2021-44228']).to eq(score: 0.97, percentile: 0.999)
      end
    end

    it 'rejects symlinked snapshots' do
      Dir.mktmpdir('epss_link_') do |root|
        Dir.mktmpdir('epss_link_target_') do |target_root|
          target = File.join(target_root, 'epss_scores.csv.gz')
          link = File.join(root, 'epss_scores.csv.gz')
          Zlib::GzipWriter.open(target) { |gz| gz.write("cve,epss,percentile\n") }
          File.symlink(target, link)

          expect { described_class.parse_epss_snapshot(link) }
            .to raise_error(ArgumentError, /not a regular file/)
        end
